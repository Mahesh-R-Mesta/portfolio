((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_3",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,E,F,G,B={
aCg(d){return new B.nY(d,null)},
nY:function nY(d,e){this.c=d
this.a=e},
Xz:function Xz(){},
Xy:function Xy(d){this.a=d}},D
A=c[0]
C=c[2]
E=c[8]
F=c[9]
G=c[12]
B=a.updateHolder(c[3],B)
D=c[11]
B.nY.prototype={
M(d){var x,w,v,u,t,s,r=null,q="confetti",p=A.bC(d,r,y.h).w.a,o=A.vx(q,"")
A.j7(q)
A.j7(q)
x=E.arH(D.mc)
x.Rf()
A.j7(q)
x=E.arI(2.356194490192345,F.l0,D.n1,x,0.1,40,!0)
w=A.tk(r,A.Ik(r,r,A.jr(C.GL,r,r,40),r,r,new B.Xy(d),r,r,r),r,r,r,20,20,r)
v=p.a
u=A.Lm(d)?v:v/1.5
t=p.b/1.5
s=this.c
return A.mU(C.bS,A.b([new A.hv(D.zh,r,r,new A.hG(o,new B.Xz(),r),r),new A.hv(C.kt,r,r,x,r),w,A.lQ(A.fA(A.a0L(new A.c8(C.hX,A.a16(s,t,v/1.5),r),s),t,u),r,r)],y.e),C.bN)}}
var z=a.updateTypes(["lX(Z)"])
B.Xz.prototype={
$1(d){var x,w="confetti"
A.j7(w)
A.j7(w)
x=E.arH(D.mc)
x.Rf()
A.j7(w)
return E.arI(0.7853981633974483,F.l0,D.n1,x,0.1,40,!0)},
$S:z+0}
B.Xy.prototype={
$0(){A.mr(this.a).Rg(null)
return null},
$S:0};(function inheritance(){var x=a.inherit
x(B.nY,A.aA)
x(B.Xz,A.hD)
x(B.Xy,A.lV)})()
A.qo(b.typeUniverse,JSON.parse('{"nY":{"aA":[],"h":[]}}'))
var y={e:A.X("l<h>"),h:A.X("ha")};(function constants(){var x=a.makeConstList
D.zh=new A.dt(-1,-1)
D.mc=new A.aL(5e6)
D.n1=A.b(x([C.f8,C.aZ,G.Ma,C.ce,G.M9]),A.X("l<p>"))})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_3",e:"endPart",h:b})})($__dart_deferred_initializers__,"xsgXKFfRUDwEU48OAnmH8EYlAr8=");